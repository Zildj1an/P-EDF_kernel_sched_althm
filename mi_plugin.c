...
static struct task_struct* mi_schedule(struct task_struct * prev) {
    ...
    */
    ...
    BUG_ON(exists && local_state->scheduled != prev);
    BUG_ON(exists && !is_realtime(prev));
    ...
    if(exists){
        ...
        /* True if current job overran its budget
           ...
           para guardar el deadline, prioridad, budget, políticas rt...
...
/*
    ...
    Según https://lwn.net/Articles/80911/ un scheduling domain es un conjunto de CPUs que comparten
    características, es bueno (eficiente) balancear entre ellos. Un domain se divide en CPU groups de varios cores
    y se considera unidad. Por tanto el balanceo es jerárquico: 1º-Qué Domain? 2º- Qué grupo?
...
static void mi_setup_domain_proc(void){
    ...
    /* Y ésto solo reserva memoria dinámica*/
